...
for($j=1;$j<=$highestRow;$j++){//从第一行开始读取数据
	...
	//插入到数据库中的对应的表中
    $sql="INSERT INTO fuzhaibiao(zichan,hangshu,zhangmian,heshi,fuzhai,hangshut,zhangmiant,heshit) VALUES (
	 '{$strs[0]}',
	 '{$strs[1]}',
	 '{$strs[2]}',
	 '{$strs[3]}',
...
function Table01(){
    ...
    foreach ($articles as $key=>$v){
        ...
        $sheet->setCellValue('A'.$currow,$v['zichan'])
            ...
            ->setCellValue('E'.$currow,$v['hangshut'])
            ->setCellValue('F'.$currow,$v['zhangmiant'])
            ->setCellValue('G'.$currow,$v['heshit']);
    }
    ...
    foreach ($articles as $key=> $v){
        ...
        {
    $phpexcel->getActiveSheet()->getStyle('A'.$currow.':G'.$currow)->getFill()->applyFromArray(array(
...
<!--	<img width="680" height="433" src="./error/wrong_format.jpg">-->
	<p><a href="http://localhost/Flex/download_fuzhaibiao.php"><img width="66" height="66" src="./error/download.jpg"></a></p>
</div>
<?php
$a=$_REQUEST["a"];
if ($a=="a")
